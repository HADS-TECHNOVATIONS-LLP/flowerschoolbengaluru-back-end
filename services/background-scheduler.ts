...
export class BackgroundScheduler {
  ...
  private interval = 30 * 60 * 1000; // 30 minutes
  ...
  private async sendStatusUpdateNotification(order: any, newStatus: string): Promise<void> {
    try {
      ...
      if ((notificationService as any).sendRawSMS) {
        await (notificationService as any).sendRawSMS(order.phone, smsMessage);
      }

      if ((notificationService as any).sendRawWhatsApp) {
        await (notificationService as any).sendRawWhatsApp(order.phone, whatsappMessage);
      }
    } catch (error) {
