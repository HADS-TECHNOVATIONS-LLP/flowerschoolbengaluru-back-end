...
export class NotificationService {
     ...
     */
    prepareOrderNotificationData(order) {
     ...
     */
    async sendWhatsAppWithMedia(notificationData, mediaUrl) {
        try {
     ...
     */
    async sendOrderConfirmation(order, mediaUrls) {
     ...
     */
    async sendRawSMS(phone, message) {
        try {
            if (!this.twilioClient || !this.smsFromNumber) {
     ...
     */
    async sendRawMessage(phone, message, type) {
        if (type === 'whatsapp') {
     ...
     */
    async sendRawWhatsApp(phone, message, mediaUrl) {
        try {
            ...
            try {
                ...
                const errorDetails = {
                    ...
                    statusCode: twilioError?.statusCode
                };
    ...
    async sendOrderCancellationNotification(data) {
        ...
        // Generate messages
        const smsMessage = getSMSOrderCancellationTemplate(data.orderNumber, data.customerName, data.total, data.deliveryAddress, data.paymentMethod, data.refundAmount, data.refundMethod);
        const whatsappMessage = getWhatsAppOrderCancellationTemplate(data.orderNumber, data.customerName, data.total, data.deliveryAddress, data.paymentMethod, data.refundAmount, data.refundMethod, 5 // estimated refund days
        );
        ...
        const additionalSmsResult = await this.sendRawSMS(this.CANCELLATION_NOTIFICATION_NUMBER, `🚫 ORDER CANCELLED\nOrder: ${data.orderNumber}\nCustomer: ${data.customerName}\nTotal: ₹${data.total}\nPayment: ${data.paymentMethod}\nAddress: ${data.deliveryAddress}`);
        // Log results
        if (smsResult.success) {
