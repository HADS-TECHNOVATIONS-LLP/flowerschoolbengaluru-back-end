...
export async function registerRoutes(app) {
    ...
    // Search API with 3 distinct scenarios
    app.get("/api/products/search", async (req, res) => {
        try {
            const { maincategory, subcategory, productname } = req.query;
            // Type cast query parameters to strings
            const mainCategoryStr = maincategory;
            const subcategoryStr = subcategory;
            const productnameStr = productname;
            // Validate that exactly one search parameter is provided
            const hasMainCategory = !!mainCategoryStr;
            const hasSubcategory = !!subcategoryStr;
            const hasProductName = !!productnameStr;
            const paramCount = [hasMainCategory, hasSubcategory, hasProductName].filter(Boolean).length;
            if (paramCount === 0) {
                return res.status(400).json({
                    success: false,
                    error: "Search parameter required",
                    message: "Please provide maincategory, subcategory, or productname to search for"
                });
            }
            if (paramCount > 1) {
                return res.status(400).json({
                    ...
                    message: "Please provide only one search parameter: maincategory, subcategory, or productname"
                });
            }
            console.log(`[SEARCH API] ===== NEW SEARCH REQUEST =====`);
            console.log(`[SEARCH API] Parameters:`, { maincategory: mainCategoryStr, subcategory: subcategoryStr, productname: productnameStr });
            let query = '';
            let queryParams = [];
            let searchType = '';
            // Scenario 1: Subcategory search only - /api/products/search?subcategory=Roses
            if (hasSubcategory) {
                searchType = 'subcategory_only';
                console.log(`[SEARCH API] SCENARIO 1: Subcategory-only search for: "${subcategoryStr}"`);
                query = `
          ...
          WHERE (
            ...
            OR (subcategory::jsonb)::text ILIKE $2
            OR subcategory::text ILIKE $3
          )
          ORDER BY id DESC
        `;
                queryParams = [subcategoryStr, `%${subcategoryStr}%`, `%${subcategoryStr.toLowerCase()}%`];
            }
            // Scenario 2: Main category search with all subcategories - /api/products/search?maincategory=occasion
            else if (hasMainCategory) {
                searchType = 'main_category_with_all_subcategories';
                console.log(`[SEARCH API] SCENARIO 2: Main category search with all subcategories for: "${mainCategoryStr}"`);
                // Get all subcategories for this main category
                const subcategoriesForCategory = getSubcategoriesForMainCategory(mainCategoryStr);
                console.log(`[SEARCH API] Found ${subcategoriesForCategory.length} subcategories for ${mainCategoryStr}:`, subcategoriesForCategory.slice(0, 5));
                if (subcategoriesForCategory.length === 0) {
                    // Fallback to basic main category search
                    query = `
            SELECT * FROM bouquetbar.products 
            WHERE (
              main_category::text ILIKE $1 
              OR main_category::jsonb ? $2
              OR main_category ILIKE $3
            )
            ORDER BY id DESC
          `;
                    queryParams = [`%${mainCategoryStr}%`, mainCategoryStr, `%${mainCategoryStr}%`];
                }
                else {
                    // Build dynamic query for main category + all its subcategories
                    const subcategoryPlaceholders = subcategoriesForCategory.map((_, index) => `subcategory::jsonb ? $${index + 4} OR subcategory ILIKE $${index + 4 + subcategoriesForCategory.length}`).join(' OR ');
                    query = `
            SELECT * FROM bouquetbar.products 
            WHERE (
              main_category::text ILIKE $1 
              OR main_category::jsonb ? $2
              OR main_category ILIKE $3
              ${subcategoryPlaceholders ? ' OR ' + subcategoryPlaceholders : ''}
            )
            ORDER BY id DESC
          `;
                    queryParams = [
                        ...
                        ...subcategoriesForCategory.map(sub => `%${sub}%`)
                    ];
                }
            }
            // Scenario 3: Product name search - /api/products/search?productname=red bouquet
            else if (hasProductName) {
                searchType = 'product_name_search';
                console.log(`[SEARCH API] SCENARIO 3: Product name search for: "${productnameStr}"`);
                query = `
          SELECT * FROM bouquetbar.products
          WHERE (
            name ILIKE $1
          ...
          ORDER BY name ASC
        `;
                queryParams = [`%${productnameStr}%`];
            }
            console.log(`[SEARCH API] Executing query with ${queryParams.length} parameters`);
            console.log(`[SEARCH API] Query:`, query.replace(/\s+/g, ' ').trim());
            console.log(`[SEARCH API] Params:`, queryParams);
            const result = await db.query(query, queryParams);
            console.log(`[SEARCH API] Database returned ${result.rows.length} products`);
            // Format products with consistent structure and all fields
            ...
            if (searchType === 'subcategory_only') {
                console.log(`[SEARCH API] RESPONSE: Subcategory-only search complete`);
                return res.status(200).json({
                    ...
                    products: products,
                    message: products.length === 0 ? `No products found for subcategory: ${subcategoryStr}` : `Found ${products.length} product(s) for subcategory: ${subcategoryStr}`
                });
            }
            else if (searchType === 'main_category_with_all_subcategories') {
                console.log(`[SEARCH API] RESPONSE: Main category with subcategories search complete`);
                // Group products by subcategory for main category search
                const subcategoriesGrouped = {};
                const subcategoryList = getSubcategoriesForMainCategory(mainCategoryStr);
                products.forEach(product => {
                    if (product.subcategory) {
                        ...
                        subcats.forEach(subcat => {
                            const cleanSubcat = subcat.replace(/["\[\]]/g, '').trim();
                            if (!subcategoriesGrouped[cleanSubcat]) {
                                subcategoriesGrouped[cleanSubcat] = [];
                            }
                            subcategoriesGrouped[cleanSubcat].push(product);
                        });
                    }
                });
                return res.status(200).json({
                    ...
                    subcategoryCount: Object.keys(subcategoriesGrouped).length,
                    message: products.length === 0 ? `No products found for main category: ${mainCategoryStr}` : `Found ${products.length} product(s) in ${Object.keys(subcategoriesGrouped).length} subcategories for main category: ${mainCategoryStr}`
                });
            }
            else if (searchType === 'product_name_search') {
                console.log(`[SEARCH API] RESPONSE: Product name search complete`);
                return res.status(200).json({
                    success: true,
                    searchType: 'product_name_search',
                    searchTerm: productnameStr,
                    totalProducts: products.length,
                    products: products,
                    message: products.length === 0 ? `No products found with name containing: ${productnameStr}` : `Found ${products.length} product(s) with name containing: ${productnameStr}`
                });
            }
        }
        catch (error) {
    ...
    app.get("/api/products", async (req, res) => {
        try {
            ...
            if (main_category || (subcategory && subcategory.toString().trim() !== '') || search) {
                ...
                let products = await storage.getProductsByCategoryAndSubcategory('', searchSubcategories.join(','), '');
    ...
    app.put("/api/admin/products/:id", async (req, res) => {
        try {
            ...
            if (raw.iscustom !== undefined)
                updates.iscustom = parseBool(raw.iscustom);
            if (raw.isCustom !== undefined)
                updates.iscustom = parseBool(raw.isCustom);
            if (raw.isbestseller !== undefined)
                updates.isbestseller = parseBool(raw.isbestseller);
            if (raw.isBestSeller !== undefined)
                updates.isbestseller = parseBool(raw.isBestSeller);
            if (raw.featured !== undefined)
                updates.featured = parseBool(raw.featured);
            ...
            if (raw.stockquantity !== undefined)
                updates.stockQuantity = Number(raw.stockquantity);
            if (raw.stockQuantity !== undefined)
                updates.stockQuantity = Number(raw.stockQuantity);
            if (raw.stock_quantity !== undefined)
                updates.stockQuantity = Number(raw.stock_quantity);
            // Handle inStock field mapping (frontend might send instock or inStock)
            if (raw.instock !== undefined)
                updates.inStock = Boolean(raw.instock);
            if (raw.inStock !== undefined)
                updates.inStock = Boolean(raw.inStock);
