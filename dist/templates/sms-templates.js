 ...
 */
export function getSMSOrderConfirmationTemplate(data) {
    ...
    if (displayItems.length > 0) {
        itemSummary = displayItems.map((item) => `${item.quantity}x ${item.name}`).join(', ');
 ...
 */
export function getSMSStatusUpdateTemplate(orderNumber, status, customerName, estimatedDelivery) {
 ...
 */
export function getSMSDeliveryReminderTemplate(orderNumber, customerName, deliveryDate, deliveryAddress) {
 ...
 */
export function getSMSPaymentReminderTemplate(orderNumber, customerName, amount, paymentMethod) {
 ...
 */
export function getSMSOrderCancellationTemplate(orderNumber, customerName, total, address, paymentMethod, refundAmount, refundMethod) {
    let refundInfo = '';
 ...
 */
export function getSMSPointsAwardedTemplate(customerName, pointsAwarded, totalPoints, orderNumber) {
 ...
 */
export function getSMSPromotionalTemplate(customerName, offer, validUntil, couponCode) {
